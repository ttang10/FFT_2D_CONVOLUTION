...
using SPMD model and using hybrid programming 
of MPI-openMP */
...
void convolution(int my_id, int p){
    ...
    if(my_id == 0){
        printf("2D convolution using SPMD model and MPI-OpenMP operations\n");
        start = MPI_Wtime();
    ...
    if(my_id == 0){
        time[13] = MPI_Wtime();
        printf("Receiving all the data of the output file(column) takes %f s.\n", time[13] - time[12]);
        ...
        printf("Total communication time of 2D convolution using MPI-OpenMP takes %f s.\n", time[13] - time[12] + time[11] - time[10] + time[7] - time[5] + time[4] - time[2] + time[1] - time[0]);
		printf("Total computing time of 2D convolution using MPI-OpenMP takes %f s.\n", time[12] - time[11] + time[10] - time[7] + time[5] - time[4] + time[2] - time[1]);
		printf("Total running time without loading/writing of 2D convolution using MPI_OpenMP takes %f s.\n", time[13] - time[0]);
        printf("Total running time of 2D convolution using MPI_OpenMP takes %f s.\n", end - start);
    }
